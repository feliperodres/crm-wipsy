...
serve(async (req) => {
  ...
  try {
    ...
    // Get the stripe_price_id for the plan
    ...
    if (planError || !planData?.stripe_price_id) {
      throw new Error(`No Stripe price found for plan: ${plan_id}`);
    }
    logStep("Plan price found", { stripe_price_id: planData.stripe_price_id });
